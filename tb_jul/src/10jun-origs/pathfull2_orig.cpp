...
nav_msgs::Path path_grid,path_visited,path_heightpath,path_frontier,path_scanzone,path_surround,path_forward,path_right,path_left;
tb_msgsrv::Paths heightpaths;
sensor_msgs::LaserScan scan_frontier;
ros::Publisher pub_path_requested,pub_path_elevated;
std_msgs::Header hdr(){
...
nav_msgs::Path find_path_elevation(nav_msgs::Path pathin){
  for(int i = 0; i < pathin.poses.size(); i++){
...
void request_pathelevation_cb(const nav_msgs::Path::ConstPtr& msg){
  pub_path_elevated.publish(find_path_elevation(*msg));
}
...
int main(int argc, char** argv)
{
  ...
  //      dt = (ros::Time::now()-t0).toSec();
      ...
      //  nav_msgs::Path path_surround_scatter = scatter_path_special(path_surround,5,5,1);
    }

  //nav_msgs::Path pathin,float m_pr_interval,int num_polys,float len_polys,float width_polys,float a1,float a2


  //  path_surround = get_path_aroundpnt2d(path_full,pos,50);
    if(draw){
      t0 = ros::Time::now();
      drawimg();
      dt = (ros::Time::now()-t0).toSec();
