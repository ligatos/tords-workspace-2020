...
int get_i(nav_msgs::Path pathin,float target_dst){
  for(int i = local_plan_i; i < pathin.poses.size(); i++){
...
int main(int argc, char **argv){
    ...
    while(ros::ok()){
     ...
     if(test_odom){
       ...
       if(global_plan.poses.size() > 1 && dst_last < 5){
         cmd_mode = "mb";
         odom.pose.pose.position = global_plan.poses[global_plan.poses.size()-1].pose.position;
         yaw_odom = get_hdng(global_plan.poses[global_plan.poses.size()-1].pose.position,global_plan.poses[global_plan.poses.size()-2].pose.position);
         got_local_plan = false;
         got_global_plan = false;
       }
       if(got_local_plan){
         cmd_mode = "mb";
         if(local_plan_i < local_plan.poses.size()){
           local_plan_i = get_i(local_plan,5);
           odom.pose.pose =  local_plan.poses[local_plan_i].pose;
           yaw_odom       = tf::getYaw(odom.pose.pose.orientation);
           if(get_dst2d(odom.pose.pose.position,pos) < 3)
            got_local_plan = false;
           ROS_INFO("Local plan: %i of %i yaw_odom: %.2f, dst: %.0f",local_plan_i,local_plan.poses.size(),yaw_odom,get_dst2d(odom.pose.pose.position,pos));
         }
         else
           got_local_plan= false;
       }
       else if(got_global_plan){
         cmd_mode = "mb";
         if(global_plan_i < global_plan.poses.size()){
           ...
           yaw_odom = get_hdng(global_plan.poses[global_plan_i].pose.position,pos);
           if(get_dst2d(odom.pose.pose.position,pos) < 3)
            got_global_plan = false;
           ROS_INFO("Global plan: %i of %i yaw_odom: %.2f, dst: %.0f",global_plan_i,global_plan.poses.size(),yaw_odom,get_dst2d(odom.pose.pose.position,pos));
         }
         else
           got_global_plan= false;
       }
    ...
    if(cmd_mode == "mb"){
      ...
      if(test_odom)
        cmd_length = (pnt2_vec - pnt1_vec).norm();
    ...
    tfpos.transform.translation.z = tfh.transform.translation.z;
      ...
      cmd.axes[0] = saturate(stride_vec.x(),10);
      cmd.axes[1] = saturate(stride_vec.y(),10);
