...
///////////********CTRL***********//////////
nav_msgs::Path pathfull,path_lo,path_mi,path_hi,path_up,path_dn,path_st,path_full;
geometry_msgs::PolygonStamped poly_frontier;
nav_msgs::Path path_targets,path_global_base,path_grid,path_visited,path_heightpath,path_frontier,path_scanzone,path_surround,path_forward,path_right,path_left;
tb_msgsrv::Paths heightpaths;
sensor_msgs::LaserScan scan_frontier;

ros::Publisher pub_cmdmb,pub_get_next_path,pub_cmd,pub_cmdpose,pub_path_requested,pub_path_elevated,pub_viewpoint,pub_path_elevation_update;
std_msgs::Header hdr(){
...
void draw_gridpath(){
  for(int i = 0; i < path_grid.poses.size(); i++){
    if(path_grid.poses[i].pose.position.z > 0){
      cv::Scalar color;
      if(path_grid.poses[i].pose.position.z > 15)
        color[2] = path_grid.poses[i].pose.position.z * 5;
      else if(path_grid.poses[i].pose.position.z > 7.5)
        color[1] = path_grid.poses[i].pose.position.z * 10;
      else
        color[1] = path_grid.poses[i].pose.position.z * 10;
      cv::circle(img,pnt2cv(path_grid.poses[i].pose.position),2,color,1);
    }
  }
}
...
bool dst_in_path_special_lim(nav_msgs::Path pathin,geometry_msgs::Point pin,float lim,float xylim,float zlim){
  float res,dst;
  if(pathin.poses.size() == 0)
    return true;
  for(int i = 0; i < pathin.poses.size(); i++){
    if( pathin.poses[i].pose.position.z <= pin.z
    &&  abs(pathin.poses[i].pose.position.z - pin.z) < zlim
    &&  abs(pathin.poses[i].pose.position.x - pin.x) < xylim
    &&  abs(pathin.poses[i].pose.position.y - pin.y) < xylim
    &&  get_dst2d(pathin.poses[i].pose.position,pin) < lim)
        return false;
  }
  return true;
}
nav_msgs::Path scatter_path_special(nav_msgs::Path pathin,float lim,float xylim,float zlim){
  nav_msgs::Path pathout;
  pathout.header = hdr();
  for(int i = 0; i < pathin.poses.size(); i++){
    if(dst_in_path_special_lim(pathout,pathin.poses[i].pose.position,lim,xylim,zlim)){
      pathout.poses.push_back(pathin.poses[i]);
    }
  }
  return pathout;
...
nav_msgs::Path get_path_aroundpnt2d(nav_msgs::Path pathin,geometry_msgs::Point midpoint,float dxy){
  nav_msgs::Path pathout;
...
nav_msgs::Path get_pathfull_aroundline(geometry_msgs::Point p2,geometry_msgs::Point p1,float dxy){
  ...
  while(cur_ray_len < tot_ray_len){
    ...
    nav_msgs::Path path_segment = get_path_aroundpnt2d(path_full,midpoint,dxy);
...
void update_gridpath(geometry_msgs::Point pnt){
  int i = xy_to_i(pnt.x,pnt.y);
  if(path_grid.poses[i].pose.position.z < pnt.z)
    path_grid.poses[i].pose.position.z = pnt.z;
  int i_at_rad = round((atan2(pnt.y,pnt.x) - scan_frontier.angle_min ) / scan_frontier.angle_increment);
  geometry_msgs::Point p0;
  float new_dst = get_dst2d(pnt,p0);
  if(scan_frontier.ranges[i_at_rad] < new_dst && new_dst < 200){
    float a0 = scan_frontier.angle_min + i_at_rad * scan_frontier.angle_increment;
    scan_frontier.ranges[i_at_rad] = new_dst;
    poly_frontier.polygon.points[i_at_rad].x = path_frontier.poses[i_at_rad].pose.position.x = scan_frontier.ranges[i_at_rad] * cos(a0);
    poly_frontier.polygon.points[i_at_rad].y = path_frontier.poses[i_at_rad].pose.position.y = scan_frontier.ranges[i_at_rad] * sin(a0);
  }
}

void create_gridpath(){
    path_grid.poses.resize(200*200);
    path_grid.header = hdr();
    int i = 0;
   for (int y = 0; y < 200; y++)
   {
     for (int x = 0; x < 200; x++)
     {
      geometry_msgs::Point pnt;
      path_grid.poses[i].pose.position.x = float(-500 + x * 5);
      path_grid.poses[i].pose.position.y = float(-500 + y * 5);
      path_grid.poses[i].pose.orientation.w = 1.0;
      i++;
     }
   }
}
...
float lowest_outofsight_infront = 10000;
  ...
  for(int i = 0; i < num_is; i++){
    ...
    while(cur_ray_len < tot_ray_len){
      ...
      nav_msgs::Path path_segment = get_path_aroundpnt2d(path_full,midpoint,stride_length);
...
void get_frontier2(float num_is,float stride_length){
  path_frontier.header = hdr();
  poly_frontier.header = hdr();
  float rads_pr_i = 2*M_PI / num_is;
  path_frontier.poses.resize(num_is);
  ...
  scan_frontier.range_max = 500;
  for(int i = 0; i < num_is; i++){
    float a0 = -M_PI + i * scan_frontier.angle_increment;
    scan_frontier.ranges[i] = 10;
    poly_frontier.polygon.points[i].z = path_frontier.poses[i].pose.position.z = 10;
    poly_frontier.polygon.points[i].x = path_frontier.poses[i].pose.position.x = scan_frontier.ranges[i] * cos(a0);
    poly_frontier.polygon.points[i].y = path_frontier.poses[i].pose.position.y = scan_frontier.ranges[i] * sin(a0);
    path_frontier.poses[i].pose.orientation = tf::createQuaternionMsgFromYaw(a0);
...
tb_msgsrv::Paths scatter_tbpaths(tb_msgsrv::Paths paths,float poses_separation){
  for(int i = 0; i < paths.paths.size(); i++){
    paths.paths[i] = scatter_path(paths.paths[i],poses_separation);
  }
  return paths;
}
//******************************************************************************************************************//
...
void update_frontier(geometry_msgs::Point pnt_in){
  int i_at_rad = round((atan2(scanpoint_mid.point.y,scanpoint_mid.point.x) - scan_frontier.angle_min ) / scan_frontier.angle_increment);
  geometry_msgs::Point p0;
  float new_dst = get_dst2d(pnt_in,p0);
  if(scan_frontier.ranges[i_at_rad] < new_dst && new_dst < 200){
    float a0 = scan_frontier.angle_min + i_at_rad * scan_frontier.angle_increment;
    scan_frontier.ranges[i_at_rad] = new_dst;
    poly_frontier.polygon.points[i_at_rad].x = path_frontier.poses[i_at_rad].pose.position.x = scan_frontier.ranges[i_at_rad] * cos(a0);
...
void midpointrequest_cb(const geometry_msgs::PointStamped::ConstPtr& msg){
  pub_path_requested.publish(get_path_aroundpnt2d(path_full,msg->point,msg->point.z));
}
...
nav_msgs::Path get_path_elevation(nav_msgs::Path pathin){
  for(int i = global_i; i < pathin.poses.size(); i++){
    pathin.poses[i].pose.position.z = path_grid.poses[xy_to_i(pathin.poses[i].pose.position.x,pathin.poses[i].pose.position.y)].pose.position.z;
  }
  return pathin;
}
...
int get_next_target(std::vector<int> vec_blacklist,nav_msgs::Path pathin,geometry_msgs::Point current_pos,float current_yaw){
  ...
  for(int i = 0; i < pathin.poses.size(); i++){
    if(!in_vec(vec_blacklist,i)){
      ...
      if(dst*dyaw < lowest_dist){
        lowest_dist = dst*dyaw;
        best_i = i;
      }
    }
  }
...
//int closest_i = getinpath_closestindex3d(pathfull,pos);
  pathfull_i = get_next_target(pathfull_ints_sent,pathfull,pos,vlp_rpy.z);
  pathfull_ints_sent.push_back(pathfull_i);
  set_target_pose(pathfull.poses[pathfull_i]);
}
...
void request_pathelevation_cb(const nav_msgs::Path::ConstPtr& msg){
  ...
  nav_msgs::Path path_elevated  = get_path_elevated(path_elevation);
...

{
  ...
  ros::Publisher pub_path_2   	 = nh.advertise<nav_msgs::Path>("/tb_world/path_2",10);
  pub_path_requested 	 = nh.advertise<nav_msgs::Path>("/tb_world/path_requested",10);
  pub_path_elevated	 = nh.advertise<nav_msgs::Path>("/tb_world/path_elevated",10);
  pub_viewpoint  = nh.advertise<geometry_msgs::Point>("/tb_world/viewpoint",10);
...
//  ros::Subscriber aa       = nh.subscribe("/tb_world/request_path_elevated",10,request_pathelevation_cb);
  ...
  vec_pathfull = get_vec_attribute(path_surround,"z");
  ...
  while(ros::ok()){
    ...
    //  ROS_INFO("PATHD_ST: %.0f",dst_target);
      pub_path_2.publish(pathfull);
    ...
    //  nav_msgs::Path pathfull_copy = constrain_path_bbpoly(path_full,poly_frontier);
      ////////////////////////////////////////
      ros::Time t0 = ros::Time::now();
      path_surround = get_path_inrad(path_full,pos,25);
      nav_msgs::Path path_pos = get_path_inrad(path_full,pos,7.5);
      pub_path_pos.publish(path_pos);
      path_forward  = cutoff_abs(path_surround,"hdng_rel",-par_hdngcutoff,par_hdngcutoff);
      path_right    = cutoff_abs(path_surround,"hdng_rel",-par_hdngcutoff*3,-par_hdngcutoff);
      path_left     = cutoff_abs(path_surround,"hdng_rel",par_hdngcutoff,par_hdngcutoff*3);
      float dt = (ros::Time::now()-t0).toSec();
  //    ROS_INFO("PATHFULL_CONSTRAIN POLY [path_surround %i path_forward %i path_right %i path_left %i poses] UPDATE: %.4f sec",path_surround.poses.size(),path_forward.poses.size(),path_right.poses.size(),path_left.poses.size(),dt);
    ...
    if(draw){
      t0 = ros::Time::now();
    //  draw_path_by_score(pathfull,vec_pathfull,2,2,2,0.4);
