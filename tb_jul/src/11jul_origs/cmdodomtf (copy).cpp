...
float current_vxy,setpoint_arm1;
...
void maxaltpos_cb(const std_msgs::UInt8::ConstPtr& msg){
...
void update_arms(){
  setpoint_arm1 		 = (rpy.y - target_arm1);
...
// --- == forward
  if((ros::Time::now() - armvec_min_max_radprs.header.stamp).toSec() < (t_int_1+t_int_2+t_int_3)){
    ...
    if(dt_arm > (t_int_1+t_int_2)){
      setpoint_arm1 = armvec_min_max_radprs.vector.y + (dt_arm-(t_int_2+t_int_1)) * armvec_min_max_radprs.vector.z;
      arm_cmd2_tilt.data = setpoint_arm1;
    }
    else if(dt_arm > t_int_1){
      setpoint_arm1 = armvec_min_max_radprs.vector.x - (dt_arm-t_int_1)           * armvec_min_max_radprs.vector.z;
      arm_cmd2_tilt.data = setpoint_arm1;
    }
    else{
      setpoint_arm1 = target_arm1                     +  dt_arm                    * armvec_min_max_radprs.vector.z;
      arm_cmd2_tilt.data = setpoint_arm1;
    }
  }
  arm_cmd2_tilt.data = saturate(arm_cmd2_tilt.data,M_PI);
  arm_cmd2_pan.data = saturate(arm_cmd2_pan.data,M_PI);
  if(setpoint_arm1 > 0)saturate(setpoint_arm1,M_PI/4);
  if(setpoint_arm1 < 0)saturate(setpoint_arm1,M_PI/2);
}
...
int main(int argc, char **argv){
  ...
  //  ros::Subscriber sxey = nh.subscribe("/tb_world/viewpoint",  100,&viewpoint_cb);
    ...
    ros::Subscriber s15s = nh.subscribe("/tb_world/maxalt_pos",10,maxaltpos_cb);
    ...
    float target_angle = get_tilt();
     ...
     float dst_last = 0;
      if(got_global_plan){
        if(global_plan.poses.size() > 0)
          dst_last = get_dst2d(global_plan.poses[global_plan.poses.size()-1].pose.position,pos);
       if(global_plan.poses.size() > 1 && dst_last < 5)
         pos_cmd = global_plan.poses[global_plan.poses.size()-1].pose.position;
       else{
         if(global_plan_i < global_plan.poses.size()){
           global_plan_i = get_i(global_plan,5);
           odom.pose.pose =  global_plan.poses[global_plan_i].pose;
           pos_cmd = global_plan.poses[global_plan_i].pose.position;
           pos_cmd.z += par_zclearing;
           odom.pose.pose.position.z = 0;
           target_angle =get_inclination_forward(global_plan);

           if(get_dst2d(odom.pose.pose.position,pos) < 3)
            got_global_plan = false;
      //     ROS_INFO("plan: %i of %i yaw_odom: %.2f, dst: %.0f",global_plan_i,global_plan.poses.size(),yaw_odom,dst_last);
         }
         else
           got_global_plan= false;
       }
       std_msgs::UInt8 global_i_msg;
       global_i_msg.data = global_plan_i;
       pub_global_i.publish(global_i_msg);
     }
     ...
     geometry_msgs::Vector3 rpy_velo;
     q.getRPY(rpy_velo.x,rpy_velo.y,rpy_velo.z);

     if(pos_cmd.z < zmax_pathpos+par_zclearing)
      pos_cmd.z = zmax_pathpos+par_zclearing;

     Eigen::Vector3f pnt1_vec(pos.x,pos.y,pos.z);
     Eigen::Vector3f pnt2_vec(pos_cmd.x,pos_cmd.y,pos_cmd.z);
     ...
     y_err = stride_vec.y();
     z_err = stride_vec.z();
     x_err = saturate(x_err,p_max);     y_err = saturate(y_err,p_max);     z_err = saturate(z_err,p_max);  t_err = saturate(setpoint_arm1 - rpy_velo.y,p_max);
 ...
 //    target_yaw.data += cmd_y*dt;
    ...
    //    ROS_INFO("tilt:   %.2f d: %.2f, i %.2f, cmd: %.2f final_cmd: %.2f",t_err,t_d,t_i,cmd_t,arm1_tilt_msg.data);
      ...
      tfvt.transform.rotation = tf::createQuaternionMsgFromRollPitchYaw(0,setpoint_arm1,cmd.axes[3]);
