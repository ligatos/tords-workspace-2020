...
void merge_update_with_grid(){
  ...
  cv::imwrite("/home/nuc/brain/fullpath/altinc"+ std::to_string(count_target_paths) + ".png",img_alt_increase);
  cv::imwrite("/home/nuc/brain/fullpath/new"+ std::to_string(count_target_paths) + "img_new" + ".png",img_new);
  cv::imwrite("/home/nuc/brain/fullpath/upd"+ std::to_string(count_target_paths) + "img_upd" + ".png",img_update);
}
...
//  else
    ...
    cv::imwrite("/home/nuc/brain/fullpath/"+s+".png",img_clear);
//  }
}
...
void update_clear(int r,int c,int z){
  for(int rr = r - 2; r < r + 2; r++){
    for(int cc = r - 2; c < c + 2; c++){
...
void update_clearpath(nav_msgs::Path pathin){
  for(int i = 0; i < pathin.poses.size(); i++){
    update_clear(y2r(pathin.poses[i].pose.position.y),x2c(pathin.poses[i].pose.position.x),int(pathin.poses[i].pose.position.z));
  }
}
