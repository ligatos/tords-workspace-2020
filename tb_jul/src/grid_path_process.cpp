...
void draw_base(){
  geometry_msgs::Point side_pos,side_pos_incl1,side_pos_incl2,side_pathpos,pos_hdng,pos_top_hdng1,pos_top_hdng2;
  float side_a1 = constrainAngle(vlp_rpy.y + M_PI/12);
  float side_a2 = constrainAngle(vlp_rpy.y - M_PI/12);
  side_pos.x = 0;
  side_pos.y = pos.z;
  side_pos_incl1.x = side_pos.x + 50 * cos(side_a1);
  side_pos_incl1.y = side_pos.y + 50 * sin(side_a1);
  side_pos_incl2.x = side_pos.x + 50 * cos(side_a2);
  side_pos_incl2.y = side_pos.y + 50 * sin(side_a2);
  float down_a1 = constrainAngle(vlp_rpy.z + M_PI/3);
  float down_a2 = constrainAngle(vlp_rpy.z - M_PI/3);
  pos_top_hdng1.x = pos.x + 50 * cos(down_a1);
  pos_top_hdng1.y = pos.y + 50 * sin(down_a1);
  pos_top_hdng2.x = pos.x + 50 * cos(down_a2);
  pos_top_hdng2.y = pos.y + 50 * sin(down_a2);
  cv::line (img, pnt2cv(pos), pnt2cv(pos_top_hdng1),get_color(200,0,0),1,cv::LINE_8,0);
  cv::line (img, pnt2cv(pos), pnt2cv(pos_top_hdng2),get_color(200,0,0),1,cv::LINE_8,0);
  cv::line (img, pnt2cv(side_pos), pnt2cv(side_pos_incl1),get_color(200,0,0),1,cv::LINE_8,0);
  cv::line (img, pnt2cv(side_pos), pnt2cv(side_pos_incl2),get_color(200,0,0),1,cv::LINE_8,0);
}
nav_msgs::Path test_path(nav_msgs::Path pathin){
  nav_msgs::Path pathout;
  pathout.header = hdr();
  std::queue<float> px_que;
  std::queue<float> py_que;
  std::queue<float> pz_que;
  std::queue<float> pyaw_que;
  ...
  ps.pose.orientation = tf::createQuaternionMsgFromYaw(vlp_rpy.z);
  for(int i = 0; i < pathin.poses.size(); i++){

    px_que.push(pathin.poses[i].pose.position.x);
    py_que.push(pathin.poses[i].pose.position.y);
    pz_que.push(pathin.poses[i].pose.position.z);
    pyaw_que.push(tf::getYaw(pathin.poses[i].pose.orientation));

    px_sum += pathin.poses[i].pose.position.x;
    py_sum += pathin.poses[i].pose.position.y;
    pz_sum += pathin.poses[i].pose.position.z;
    pyaw_sum += tf::getYaw(pathin.poses[i].pose.orientation);
    ROS_INFO("Pathin: %i %.0f %.0f %.0f",i,pathin.poses[i].pose.position.x,pathin.poses[i].pose.position.y,pathin.poses[i].pose.position.z);
    if(pz_que.size() > que_size){
      px_sum -= px_que.front();
      py_sum -= py_que.front();
      pz_sum -= pz_que.front();
      pyaw_sum -= pyaw_que.front();

      float px = px_sum / que_size;
      float py = py_sum / que_size;
      float pz = pz_sum / que_size;
      float pyaw = pyaw_sum / que_size;
      ROS_INFO("Pathin: %i %.0f %.0f %.0f %.2f",i,px,py,pz,pyaw);
      px_que.pop();
      py_que.pop();
      pz_que.pop();
      pyaw_que.pop();
      float dz = pz - ps.pose.position.z;
      float dy = py - ps.pose.position.y;
      float dx = px - ps.pose.position.x;
      float ps_yaw = tf::getYaw(ps.pose.orientation);
      float dyaw = get_shortest(pyaw,ps_yaw);
      float rads_pr_pose = M_PI/2;
      ps.pose.position.z = pz;
      ps.pose.position.y = py;
      ps.pose.position.x = px;
      ROS_INFO("Ps: %.0f %.0f %.0f %.2f",px,py,pz,pyaw);
      ps.pose.orientation = tf::createQuaternionMsgFromYaw(pyaw);
      pathout.poses.push_back(ps);
    }
  }
  return pathout;
}
void draw_process_path(nav_msgs::Path pathin,std::vector<float> dists,std::vector<float> hdngs,std::vector<float> incls){
  img_blank.copyTo(img);
  geometry_msgs::Point side_pathpos,  side_drawpnt,  top_drawpnt;
  int pathlen = 0;
  side_pathpos.x = 0;
  draw_base();
  for(int  i = 0; i< dists.size(); i++){
    side_pathpos.x += dists[i];
    side_pathpos.y = pathin.poses[i].pose.position.z;
    side_drawpnt.x = side_pathpos.x + 8 * cos(incls[i]);
    side_drawpnt.y = side_pathpos.y + 8 * sin(incls[i]);
    top_drawpnt.x = pathin.poses[i].pose.position.x + 8 * cos(hdngs[i]);
    top_drawpnt.y = pathin.poses[i].pose.position.y + 8 * sin(hdngs[i]);
    cv::line (img, pnt2cv(side_pathpos), pnt2cv(side_drawpnt),get_color(0,0,100),1,cv::LINE_8,0);
    cv::circle(img,pnt2cv(side_pathpos),1,get_color(0,100,100),1);
    cv::line (img, pnt2cv(pathin.poses[i].pose.position), pnt2cv(top_drawpnt),get_color(100,0,0),1,cv::LINE_8,0);
    cv::circle(img,pnt2cv(pathin.poses[i].pose.position),1,get_color(100,100,0),1);
  }
  count_target_paths++;
  std::string s = std::to_string(count_target_paths)+"test";
  cv::imwrite("/home/nuc/brain/fullpath/"+s+ ".png",img);
}
nav_msgs::Path process_path(nav_msgs::Path pathin){
  std::vector<float> dsts;
  std::vector<float> hdns;
  std::vector<float> incl;
  if(pathin.poses.size() == 0)
    return pathin;
  pathin.poses[0].pose.position.z = get_zmax_grid(pathin.poses[0].pose.position,5);
  for(int i = 1; i < pathin.poses.size()-1; i++){
    float dist = get_dst2d(pathin.poses[i].pose.position,pathin.poses[i+1].pose.position);
    pathin.poses[i+1].pose.position.z = get_zmax_grid(pathin.poses[i+1].pose.position,5);
    pathin.poses[i].pose.orientation = tf::createQuaternionMsgFromYaw(get_hdng(pathin.poses[i+1].pose.position,pathin.poses[i].pose.position));
    hdns.push_back(get_hdng(pathin.poses[i+1].pose.position,pathin.poses[i].pose.position));
    incl.push_back(atan2(pathin.poses[i+1].pose.position.z-pathin.poses[i].pose.position.z,dist));
    dsts.push_back(dist);
  //  pathin.poses[i].pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(0,-incl,hdng_last);
  }
  draw_process_path(pathin,dsts,hdns,incl);
  return pathin; //float vz   = (pathin.poses[i].pose.position.z - pathin.poses[i].pose.position.z);
  //float vy   = (pathin.poses[i].pose.position.y - pathin.poses[i].pose.position.y);
  //float vx   = (pathin.poses[i].pose.position.x - pathin.poses[i].pose.position.x);
}
void do_process_and_draw(nav_msgs::Path pathin){
  img_blank.copyTo(img);
  nav_msgs::Path path_new = test_path(process_path(pathin));
  pub_path_elevated.publish(path_new);
  draw_path(path_new,get_color(0,200,0),0);
  nav_msgs::Path path_new2 = process_path(path_new);
  draw_path(path_new2,get_color(200,200,0),2);
  std::string s = std::to_string(count_target_paths)+"testrea";
  cv::imwrite("/home/nuc/brain/fullpath/"+s+ ".png",img);
}
...
//  pathfull = create_path(area_sidelength,total_sidelength);
  ...
  while(ros::ok()){
    ...
    if(got_map){
      ...
      pub_path_elevation_update.publish(path_elevation_edto);
