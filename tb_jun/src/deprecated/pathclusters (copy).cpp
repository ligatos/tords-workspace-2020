...
nav_msgs::Path superpath_down,superpath_sides;

nav_msgs::Path path_visited_inpoly,path_vlp_full,path_candidates_sides,path_candidates,path_visited,path_candidates2d,path_floor,path_vlp;
...
std::vector<nav_msgs::Path> paths_sides;
std::vector<nav_msgs::Path> paths_down;
...
float get_path_candidates(){
  ...
  //      if(is_within_incline(ps.pose.position,par_delta_incline) && in_poly(poly_vlp,ps.pose.position.x,ps.pose.position.y)){
        if(is_within_incline(ps.pose.position,par_delta_incline) && in_poly(poly_vlp_ext,ps.pose.position.x,ps.pose.position.y) && dst_point_in_path_lim(path_visited_inpoly,ps.pose.position,8)){ //!dst_point_in_path_lim(path_vlp,ps.pose.position,8) &&
  ...
  std::vector<std::vector<int>> clusters = get_neighbour_clusters(path_candidates,4);
  paths_down = order_clusters(paths_from_clusters(path_candidates,clusters));
  paths_down = scatter_clusters(paths_down,3);
  float dt = (ros::Time::now() - t0).toSec();
  return dt;
}
...
float get_path_candidates_sides(){
  ...
  for(int z = int(round(bbmin_vlp.z)); z < int(round(bbmax_vlp.z)); z++){
    for(int y = ymin; y < ymax; y++){
      for(int x = xmin; x < xmax; x++){
        ...
        ps.pose.position.z = z;
    //    if(is_within_incline(ps.pose.position,par_delta_incline) && in_poly(poly_vlp,ps.pose.position.x,ps.pose.position.y)){
        if(is_within_incline(ps.pose.position,par_delta_incline) && in_poly(poly_vlp,ps.pose.position.x,ps.pose.position.y) && dst_point_in_path_lim(path_visited_inpoly,ps.pose.position,8)){// !dst_point_in_path_lim(path_vlp,ps.pose.position,8) ){
          octomap::point3d p(x,y,z);
          octomap::point3d closestObst;
          float d;
          edf_ptr.get()->getDistanceAndClosestObstacle(p,d,closestObst);
  ...
  std::vector<std::vector<int>> clusters     = get_neighbour_clusters(path_candidates_sides,4);
  paths_sides =  order_clusters(paths_from_clusters(path_candidates_sides,clusters));
  paths_sides = scatter_clusters(paths_sides,3);
...
int main(int argc, char **argv)
{
  ...
  ros::Publisher pub_candidates_sides = nh.advertise<nav_msgs::Path>("/tb_path/sides",100);
  ...
  ros::Publisher pub_superpath_sides = nh.advertise<nav_msgs::Path>("/tb_path/superpath_sides",100);
  ros::Publisher pub_path_side = nh.advertise<nav_msgs::Path>("/tb_clusters/sides",10);
  ...
  while(ros::ok()){
    ...
    if(got_map){
      ...
      if(get_floor){
        ...
        superpath_down.poses.resize(0);
        superpath_sides.poses.resize(0);
        superpath_down  = create_superpath_clusters(clusters_down);
        superpath_sides = create_superpath_clusters(clusters_sides);
        pub_superpath_down.publish(superpath_down);
        pub_superpath_sides.publish(superpath_sides);
        pub_path_side.publish(cluster_sides);
        pub_path_down.publish(cluster_down);

        ROS_INFO("drawn in %.4f",dt);
