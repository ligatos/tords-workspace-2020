...
img_blank.copyTo(img);
count++;
cv::imwrite("/home/nuc/brain/pathclusters/"+std::to_string(count)+".png",img);
float dt = (ros::Time::now() - t1).toSec();

nav_msgs::Path get_data_path_from_pose(nav_msgs::Path )
nav_msgs::Path analyse_path(nav_msgs::Path path_targets_new){
  count_target_paths++;
  img_blank.copyTo(img);

  cv::Scalar color_pos = get_color(200,200,0);
  nav_msgs::Path path_targets_remaining   = get_targets_remaining();
  geometry_msgs::Point p0;

  draw_path_at_img(path_targets_remaining,p0,false,true,false,true,false,color_pos,1);
  draw_path_at_img(path_targets,p0,false,false,false,false,true,color_pos,1);
  cv::circle(img,pnt2cv(pos),2,color_pos,1);

  for(int i = 0; i < path_targets_remaining.poses.size(); i++){
    float yaw  = tf::getYaw(path_targets_remaining.poses[i].pose.orientation);
    float hdng,hdng_pos;
    dst_pos = get_dst3d(path_targets_remaining.poses[i].pose.position,pos);
    hdng_pos = get_hdng(path_targets_remaining.poses[i].pose.position,pos);

    if(i > 0){
      hdng = get_hdng(path_targets_remaining.poses[i].pose.position,path_targets_remaining.poses[i-1].pose.position);
      dst  = get_dst3d(path_targets_remaining.poses[i].pose.position,path_targets_remaining.poses[i-1].pose.position);
    }
    else{
      hdng = hdng_pos;
      dst  = dst_pos;
    ...
    for(int i = 0; i < pathin.poses.size(); i++){
      float yaw  = tf::getYaw(pathin.poses[i].pose.orientation);
      float hdng,hdng_pos;
      dst_pos = get_dst3d(pathin.poses[i].pose.position,pos);
      hdng_pos = get_hdng(pathin.poses[i].pose.position,pos);

      if(i > 0){
        hdng = get_hdng(pathin.poses[i].pose.position,pathin .poses[i-1].pose.position); 
        dst  = get_dst3d(pathin.poses[i].pose.position,pathin .poses[i-1].pose.position); 
      }
      else{
        hdng = hdng_pos;
        dst  = dst_pos;
      }
    ROS_INFO("Current_target[%i/%i] dst_pos: %.0f hdbg_pos: %.2f dst: %.0f hdbg: %.2f ",i,path_targets_remaining.poses.size(),dst_pos,hdng_pos,dst,hdng);
  }
...
nav_msgs::Path score_path(nav_msgs::Path pathin){
  for(int i = 0; i < pathin.poses.size(); i++){
    float dst = pathin.poses
  }
  float best_dst0 = 30;
  float best_hdng = 0;
  float best_deltadst = 0;
...
std::vector<nav_msgs::Path> get_cluster_from_superpath(nav_msgs::Path pathin){
  ...
  for(int i = 0; i < pathin.poses.size(); i++){
    if(pathin.poses[i].header.frame_id == "next_path"){
      if(path_current.poses.size() > 0){
        ROS_INFO("path cluster: %i holdes %i poses",Path_clusters_out.size(),path_current.poses.size());
  ...
  ROS_INFO("Total clusters: %i from %i poses in",Path_clusters_out.size(),pathin.poses.size());
