...
geometry_msgs::Point scanpoint_actual,scanpoint_actual_lo,scanpoint_actual_hi;
...
void closest_pos_cb(const geometry_msgs::PointStamped::ConstPtr& msg){
  if(msg->header.frame_id != "map")
    cv::circle(img,pnt2cv(msg->point),2,get_color(0,0,200),1);
}
...
geometry_msgs::PolygonStamped create_bbpoly(float dmin,float dmax,float z,float a0,float an){
  ...
  poly.header = hdr();
  poly.polygon.points[0].x = pos.x + dmin * cos(a0);
  poly.polygon.points[0].y = pos.y + dmin * sin(a0);
  poly.polygon.points[0].z = z;

  poly.polygon.points[1].x = pos.x + dmin * cos(an);
  poly.polygon.points[1].y = pos.y + dmin * sin(an);
  poly.polygon.points[1].z = z;

  poly.polygon.points[2].x = pos.x + dmax * cos(an);
  poly.polygon.points[2].y = pos.y + dmax * sin(an);
  poly.polygon.points[2].z = z;

  poly.polygon.points[3].x = pos.x + dmax * cos(a0);
  poly.polygon.points[3].y = pos.y + dmax * sin(a0);
...
//  work_front_scan(path_all,vlp_rpy.z,front_cutoff);
  //work_front_scan(path_all,vlp_rpy.z-front_cutoff,front_cutoff);
  //work_front_scan(path_all,vlp_rpy.z+front_cutoff,front_cutoff);
...
/*
  geometry_msgs::PolygonStamped  poly_mi_mi = path_to_bb_poly(path_mi_mi,false);
  geometry_msgs::PolygonStamped  poly_mi_ri = path_to_bb_poly(path_mi_ri,false);
  geometry_msgs::PolygonStamped  poly_mi_le = path_to_bb_poly(path_mi_le,false);

  std::vector<float> vals_mi = get_vec_attribute(path_mi_mi,"z");
  std::vector<float> vals_ri = get_vec_attribute(path_mi_ri,"z");
  std::vector<float> vals_le = get_vec_attribute(path_mi_le,"z");

  draw_path_by_score(path_mi_mi,vals_mi,2,2,0,1.0);
  draw_path_by_score(path_mi_ri,vals_ri,1,1,0,1.0);
  draw_path_by_score(path_mi_le,vals_le,1,1,0,1.0);
  draw_poly(poly_mi_mi,get_color(0,0,200));
  draw_poly(poly_mi_ri,get_color(0,200,0));
  draw_poly(poly_mi_le,get_color(0,200,0));
  drawimg(hdng_cutoff,pos_yaw,"testing");*/
}
