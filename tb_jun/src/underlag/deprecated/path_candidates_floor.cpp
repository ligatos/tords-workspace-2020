...
void update_path_floor(int n_grids, float grid_sidelength)
{
...
//  int min_zlvl = 1;
  //int max_zlvl = ;
  int x0 = int(round(pos.x)) - grid_sidelength * n_grids / 2 + grid_sidelength / 2;
  int y0 = int(round(pos.y)) - grid_sidelength * n_grids / 2 + grid_sidelength / 2;
  ...
  for(int yn = 0; yn < n_grids; yn++){
    for(int xn = 0; xn < n_grids; xn++){
      ...
      update_edto(pmid,pos.z-bbmin_octree.z,grid_sidelength,bbmin_octree.z,pos.z,par_unknownAsOccupied);
      float d;
...
void checktf(){
  ...
  if(abs(last_yaw - pos_yaw) > 1 || get_dst3d(pos,last_pos) > 5){
    get_floor = true;
    update_path_floor(5,3);
  }
}

int main(int argc, char **argv)
{
  ros::init(argc, argv, "tb_pathfloor_node");
  ros::NodeHandle nh;
  ...
  pub_floor = nh.advertise<nav_msgs::Path>("/tb_path_floor",100);
