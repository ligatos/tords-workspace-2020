...
geometry_msgs::PoseStamped eval_areas_cleared(nav_msgs::Path pathin,bool draw){
  ...
  for(int i = 0; i < pathin.poses.size(); i++){
    ...
    ROS_INFO("Eval_area: xyz: %.2f %.2f %.2f",eval_area.x,eval_area.y,eval_area.z);
...
path_full.header = hdr();
  ...
  //  cv::imwrite("/home/nuc/brain/weights/"+std::to_string(count_target_paths)+"_weights.png",img);
      ...
      if(dash_stage == 0){
        ...
        if(target_index > 0 && target_index < path_full.poses.size()){
          if(target_index == path_full.poses.size()-1 && target_index > 0)
            target_index = -1;
          float hdng_target = get_hdng(path_full.poses[target_index+1].pose.position,pos);
          float target_dst  = get_dst2d(path_full.poses[target_index+1].pose.position,pos);
          ROS_INFO("BEHAV-Main: path_full[%i/%i],",target_index,path_full.poses.size(),hdng_target,target_dst);
          dashnumber++;
