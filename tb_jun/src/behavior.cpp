...
void draw_target_sitrep(){
  ...
  img_blank.copyTo(img);
  draw_pose(target.pose.position,tf::getYaw(target.pose.orientation),5,get_color(200,200,0));
  draw_pose(target_last.pose.position,tf::getYaw(target_last.pose.orientation),5,get_color(100,100,0));

  draw_pose(pos,target_hdng,5,get_color(0,200,200));
  draw_path_at_img(path_side_best,base_pose.pose.position,true,false,true,false,false,get_color(150,0,0),1);
  draw_path_at_img(path_down_best,base_pose.pose.position,false,false,true,false,false,get_color(0,150,0),1);
  cv::imwrite("/home/nuc/brain/"+std::to_string(count_target_paths)+"_targets.png",img);
  draw_clusters(paths_active_side,"lines_p0",false,"side");
  draw_clusters(paths_active_down,"lines_p0",false,"down");
  cv::imwrite("/home/nuc/brain/"+std::to_string(count_target_paths)+"_targetsclusters.png",img);
...
void set_target_pose(geometry_msgs::PoseStamped new_target_pose){
  ...
  draw_target_sitrep();
  if(count_target_paths > 20)
    pub_cmdexplore.publish(createpoly_square(new_target_pose.pose.position,50));
  float yawnew = get_hdng(new_target_pose.pose.position,pos);
  new_target_pose.pose.orientation = tf::createQuaternionMsgFromYaw(yawnew);
...
void set_target_path(nav_msgs::Path pathin){
  ...
  if(inspection_type == "down"){
    geometry_msgs::PoseStamped ps_target;
    float zmx                    = get_zmax_ni(pathin,pathin.poses.size());
    int ave_pnt_i                = get_closest_i(pathin,get_ave_pnt_ni(pathin,pathin.poses.size()));
    ps_target = pathin.poses[ave_pnt_i];
    ps_target.pose.position.z = zmx + 3;
    set_target_pose(ps_target);
  }
  else{
...
nav_msgs::Path get_next_candidates(tb_msgsrv::Paths pathsin,geometry_msgs::PoseStamped pose_comparison){
  ...
  count_target_paths++;
  if(path_is_down)
    cv::imwrite("/home/nuc/brain/pathclusters/"+std::to_string(count_target_paths)+"_down_comparison.png",img);
  else
...
void draw_paths(){
  ...
  draw_path_at_img(path_side_best,base_pose.pose.position,false,true,false,false,true,get_color(0,100,0),1);
  draw_path_at_img(path_down_best,base_pose.pose.position,false,false,false,false,true,get_color(100,0,0),1);
  ...
  nav_msgs::Path path_side_best_hdng = cutoff_abs(sort_path(path_side_best,"hdng_abs"),40);
  nav_msgs::Path path_down_best_hdng = cutoff_abs(sort_path(path_down_best,"hdng_abs"),40);
  nav_msgs::Path path_obs_hdng = cutoff_abs(sort_path(path_obs,"hdng_abs"),40);
  ...
  if(path_obs.poses.size() > 0){
    ...
    zmx_do = zmx_o;
    closest_obs = path_obs.poses[get_closest_i(path_obs_hdng,pos)].pose.position;
    farthest_obs = path_obs.poses[get_farthest_i(path_obs_hdng,pos)].pose.position;
  ...
  if(path_side_best_hdng.poses.size() > 0){
    zmx_s = get_zmax(path_side_best_hdng);
    farthest_side = path_side_best_hdng.poses[get_farthest_i(path_side_best_hdng,pos)].pose.position;
    closest_side  = path_side_best_hdng.poses[get_closest_i(path_side_best_hdng,pos)].pose.position;
  ...
  if(path_down_best_hdng.poses.size() > 0){
    zmx_d = get_zmax(path_down_best_hdng);
    zmx_do = fmax(zmx_d,zmx_o);
    farthest_down = path_down_best_hdng.poses[get_farthest_i(path_down_best_hdng,pos)].pose.position;
    closest_down  = path_down_best_hdng.poses[get_closest_i(path_down_best_hdng,pos)].pose.position;
  ...
  ROS_INFO("poly_area: %.0f pathsizes: RSIDE: %i RDOWN: %i vlp: %i clear: %i obs: %i ",poly_cleared_centroid_area,path_side_best_hdng.poses.size(),path_down_best_hdng.poses.size(),
  ...
  draw_path_at_img(path_side_best_hdng,base_pose.pose.position,false,true,false,false,true,get_color(0,100,0),1);
  draw_path_at_img(path_down_best_hdng,base_pose.pose.position,false,false,false,false,true,get_color(100,0,0),1);
  draw_path_at_img(path_obs_hdng,base_pose.pose.position,false,true,true,false,false,get_color(0,0,100),1);
...
void draw_vlp_view(){
  ...
  nav_msgs::Path path_obs_hdng = cutoff_abs(sort_path(path_obs,"hdng_abs"),20);
  nav_msgs::Path path_clear_vlp2 = cutoff_abs(sort_path(path_clear_vlp,"hdng_abs"),20);
  for(int i = 0; i < path_obs_hdng.poses.size(); i++){
    pnt_path.y = path_obs_hdng.poses[i].pose.position.z;
    pnt_path.x = get_dst2d(path_obs_hdng.poses[i].pose.position,pos);
    cv::circle(img,pnt2cv(pnt_path),3,get_color(55,55,0),1);
  }
  for(int i = 0; i < fmin(path_clear_vlp.poses.size(),2); i++){
    pnt_path.y = path_clear_vlp.poses[i].pose.position.z;
    pnt_path.x = get_dst2d(path_clear_vlp.poses[i].pose.position,pos);
    cv::circle(img,pnt2cv(path_clear_vlp.poses[i].pose.position),3,get_color(0,55,55),1);
  }
...
geometry_msgs::PoseStamped eval_areas_cleared(nav_msgs::Path pathin,bool draw){
  ...
  if(draw)
    cv::imwrite("/home/nuc/brain/"+std::to_string(count_target_paths)+"_area_eval.png",img);
...
void hdngclear_cb(const nav_msgs::Path::ConstPtr& msg){
  ...
  if(msg->poses.size() > 0){
    if(dash_stage == 1){
      ...
      if(offset_tot < 0.2){
        ...
        cv::imwrite("/home/nuc/brain/"+std::to_string(count_target_paths)+"_dashed.png",img);
      }
      else{
  ...
  //  cv::imwrite("/home/nuc/brain/"+std::to_string(count_target_paths)+"_evaled.png",img);
  }
  ROS_INFO("eval done");
}
    /*
...
int main(int argc, char** argv)
{
  ...
  while(ros::ok()){
    ...
    if(mainstate != 1)
      ...
      if(dst_to_target < 5){
        img_blank.copyTo(img);
        float dt_down = (ros::Time::now() - pose_down.header.stamp).toSec();
        float dt_side = (ros::Time::now() - pose_side.header.stamp).toSec();
        float hdng_s = get_hdng(pose_side.pose.position,pos);
        float dhdng_s = get_shortest(hdng_s,tf::getYaw(base_pose.pose.orientation));
        float hdng_d = get_hdng(pose_down.pose.position,pos);
        float dhdng_d = get_shortest(hdng_d,tf::getYaw(base_pose.pose.orientation));
        if(dhdng_s < 0)
         dhdng_s *= -1;
        if(dhdng_d < 0)
          dhdng_d *= -1;
        float dst_s  = get_dst2d(pose_side.pose.position,pos);
        float dst_d  = get_dst2d(pose_down.pose.position,pos);
        float score_s = dhdng_s * dst_s * dt_side;
        float score_d = dhdng_d * dst_d * dt_down;
        ROS_INFO("Dt_down: %.2f dt_side: %.2f hdng_s: %.2f hdng_d: %.2f, dst_s: %.0f dst_d: %.0f score: %.4f vs %.4f",dt_down,dt_side,hdng_s,hdng_d,dst_s,dst_d,score_s,score_d);

        draw_pose(base_pose.pose.position,tf::getYaw(base_pose.pose.orientation),5,get_color(200,0,0));
        draw_pose(pose_down.pose.position,get_hdng(pose_down.pose.position,pos),5,get_color(200,0,0));
        draw_pose(pose_side.pose.position,tf::getYaw(pose_side.pose.orientation),5,get_color(200,0,0));
        if(score_s < score_d){
        ...
        cv::imwrite("/home/nuc/brain/"+std::to_string(count_target_paths)+"_poses.png",img);
      }
    }
		else if(use_test){
